...
현재 구현은 Foundation과 공통 요소에 이어 계정·온보딩·홈, 옷장 목록·등록·상세, FitLab 입력·추천·리포트·히스토리, 마이페이지·설정까지 진행했다. 앱 전체 포팅 또는 출시 완료를 의미하지 않는다.
...
| Phase 4 Screen Porting | 일부 구현 | 스플래시·인증·3단계 온보딩·원본 약관·main04·옷장·FitLab·마이페이지, 세션에 따른 제품 라우팅과 기능별 API 연결 |
...
2. 후속 화면 이식: Google Play 실타래 충전/결제·보상형 광고·공유/딥링크. 마이페이지·설정은 완료했으며 충전은 잔액 조회와 준비 안내까지만 제공.
3. API DTO/서비스를 각 기능의 원본 계약에 맞춰 확장하고 실제 서버 E2E 실행.
