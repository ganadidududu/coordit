...
또한 최근 사용자 피드백을 카테고리별 보정 프로필로 요약하되, 충분한 신뢰도 기준을 통과한 피드백만 다음 추천의 offset과 부위별 가중치에 제한적으로 반영한다.
...
피드백 프로필이 있으면 `μₖ`와 `Wₖ`는 추천 전 보정된다.

```text
adjusted_μₖ = μₖ + feedback_offsetₖ
adjusted_Wₖ = normalize(Wₖ × feedback_weight_multiplierₖ)
diffₖ = yₖ - adjusted_μₖ
```

예를 들어 사용자가 같은 카테고리에서 반복적으로 `too_small` 피드백을 남기면 해당 카테고리의 기준 프로필이 조금 더 큰 실측값을 선호하도록 이동한다.
...
`mvp_rule_v1_5`부터 confidence 판단 근거는 `scoreExplanation`과
`confidenceBreakdown`에 선택 메타데이터로 저장될 수 있다. 이 메타데이터는
confidence와 리포트 설명을 돕기 위한 것이며, 기존 `fit_score` 계산값을
LLM이나 리포트 생성 단계에서 다시 계산하지 않는다.
...
### 16.1 MVP+1: 전체 핏 방향 보정

`actualFitLabel`을 사용해 사용자/카테고리별 전체 선호 여유분을 만든다.
...
전체 핏 방향 보정은 해당 카테고리의 주요 측정값 전체에 적용된다. 상의는 어깨, 가슴단면, 총장, 소매길이에 적용하고 하의는 허리단면, 엉덩이단면, 밑위, 아웃심에 적용한다.

### 16.2 MVP+2: 부위별 피드백 보정

`partFeedback`을 사용해 특정 부위의 offset과 weight multiplier를 계산한다.
...
- `too_small`, `slightly_small`은 해당 부위의 선호 실측값을 크게 이동시킨다.
- `too_large`, `slightly_large`는 해당 부위의 선호 실측값을 작게 이동시킨다.
- 문제가 반복된 부위는 weight multiplier를 올려 다음 추천에서 더 민감하게 본다.
- 최종 가중치 합은 1로 정규화한다.

### 16.3 Feedback Profile

추천 실행 시 최근 피드백은 `feedbackProfile`로 요약된다.
...
이 프로필은 추천 결과의 `result_details.feedbackProfile`에도 저장되어 어떤 보정이 적용됐는지 추적할 수 있다.

신뢰도 기준:

- 카테고리 전체 보정은 기본적으로 사용 가능한 최근 피드백 5개 이상에서 적용한다.
- 부위별 weight multiplier는 해당 측정 항목 피드백 3개 이상에서 적용한다.
- 오래된 피드백은 최근 피드백보다 낮게 반영한다.
- 구매 사이즈, 클릭, 구매 로그가 있으면 보정 신뢰도 판단의 보조 신호로만 사용한다.
- 기준 미달 또는 상충 피드백은 기록하지만 강한 offset으로 적용하지 않는다.

현재 단계는 rule-based personalization이다. ML 학습은 아직 수행하지 않는다.
...
- 사용자 피드백은 최근 카테고리 단위 보정에는 사용되지만, 아직 ML 학습에는 사용되지 않는다.
- 브랜드별 사이즈 보정이 없다.
- OCR/URL 파싱 결과의 신뢰도는 confidence와 리포트 caveat에만 반영되며, 추천 점수에는 아직 반영되지 않는다.
