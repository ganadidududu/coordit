...
const runExactPackageCommand = async (): Promise<CliResult> => {
  ...
  const child = spawn(
    "npm",
    ["run", "--silent", "probe:profile-styling-schema-reconciliation:connection"],
...
const assertExactCliFailure = async (): Promise<void> => {
  ...
  assert.equal(
    result.stderr,
    "profile_styling_connection_probe status=failed category=operator_tls_configuration"
  );
};
...
const assertConnectionOnlySuccess = async (): Promise<void> => {
  ...
  const exitCode = await executeProfileStylingOperatorConnectionProbe(
    operatorEnvironment,
    {
  ...
  assert.deepEqual(stdoutLines, ["profile_styling_connection_probe status=connected\n"]);
...
const assertConnectionFailureIsRedacted = async (): Promise<void> => {
  ...
  assert.deepEqual(stderrLines, [
    "profile_styling_connection_probe status=failed category=connection_failed\n"
  ]);
};
...
const run = async (): Promise<void> => {
  ...
  process.stdout.write([
    "profile_styling_connection_probe assertions=pass",
...
void run().catch((error: unknown) => {
  if (error instanceof Error) {
    ...
    process.stderr.write("Unknown profile/styling connection probe test failure\n");
  }
  process.exit(1);
});
